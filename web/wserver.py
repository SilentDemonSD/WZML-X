...
@app.route('/app/files/<string:torrent_id>?pin_code=<string:pin_code>')
def list_torrent_contents(torrent_id, pin_code):
    if not validate_pin_code(torrent_id, pin_code):
        return "<h1>Incorrect pin code</h1>"

    if len(torrent_id) > 20:
        with qbClient(host="localhost", port="8090") as client:
            try:
                res = client.torrents_files(torrent_hash=torrent_id)
                cont = make_tree(res)
        ...
        try:
            res = aria2.client.get_files(torrent_id)
            cont = make_tree(res, True)
        except KeyError:
            return abort(404)

    return render_template('list_torrent_contents.html', My_content=cont[0], form_url=url_for('set_priority', torrent_id=torrent_id, pin_code=pin_code))

@app.route('/app/files/<string:torrent_id>/priority', methods=['POST'])
def set_priority(torrent_id, pin_code):
    if not validate_pin_code(torrent_id, pin_code):
        return "<h1>Incorrect pin code</h1>"
...
def validate_pin_code(torrent_id, pin_code):
    pincode = ""
    for nbr in torrent_id:
        if nbr.isdigit():
            pincode += str(nbr)
        if len(pincode) == 4:
            break

    return pin_code == pincode
...
<body>
    ...
    <div class="content">
        {{ My_content }}
        <form method="POST" action="{{ url_for('set_priority', torrent_id=torrent_id, pin_code=pin_code) }}">
