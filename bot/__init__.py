...
from bot.pyro_addons import Conversation
...
if ospath.exists('WZML-Pyro.session'):
    osremove('WZML-Pyro.session')
if ospath.exists('WZML-Pyro.session-journal'):
    osremove('WZML-Pyro.session-journal')
...
if not ospath.isdir(path):
    mkdir(path)
log_photo_dir = path + TGH_THUMB.split('/')[-1]
urlretrieve(TGH_THUMB, log_photo_dir)
Image.open(log_photo_dir).convert("RGB").save('Thumbnails/weeb.jpg', "JPEG")
osremove(log_photo_dir)

if DEF_THUMB:
    def_photo_dir = path + DEF_THUMB.split('/')[-1]
    urlretrieve(DEF_THUMB, def_photo_dir)
    Image.open(def_photo_dir).convert("RGB").save(
        'Thumbnails/default.jpg', "JPEG")
    osremove(def_photo_dir)
...
bot = Client(name='WZML-Pyro', api_id=TELEGRAM_API, api_hash=TELEGRAM_HASH, bot_token=BOT_TOKEN, parse_mode=enums.ParseMode.HTML, plugins={"root": "bot/modules"})
Conversation(bot)
...
try:
    ...
    if len(USER_SESSION_STRING) != 0:
        premium_session = Client('WZML-Premium', api_id=TELEGRAM_API, api_hash=TELEGRAM_HASH,
    ...
    if not premium_session:
        ...
        if (premium_session.get_me()).is_premium:
            if not LEECH_LOG:
                LOGGER.error(
                    "You must set LEECH_LOG for uploads. Exiting Now...")
                try:
            ...
            LOGGER.info(
                "Telegram Premium Detected! Leech Limit upgraded to 4GB")
...
rss_session = Client(name='rss_session', api_id=(TELEGRAM_API), api_hash=TELEGRAM_HASH, session_string=RSS_USER_SESSION_STRING,
                     parse_mode=enums.ParseMode.HTML, no_updates=True) if len(RSS_USER_SESSION_STRING) != 0 else None
...
MEDIAINFO_MODE = environ.get('MEDIAINFO_MODE', '')
MEDIAINFO_MODE = MEDIAINFO_MODE.lower() == 'true'
...
if len(SHORTENER) == 0 or len(SHORTENER_API) == 0:
    SHORTENER = ''
    SHORTENER_API = ''
SHORTENER = (SHORTENER.replace("'", '').replace('"', '').replace(
    '[', '').replace(']', '').replace(",", "")).split()
SHORTENER_API = (SHORTENER_API.replace("'", '').replace(
...
PICS = (PICS.replace("'", '').replace('"', '').replace(
    '[', '').replace(']', '').replace(",", "")).split()
