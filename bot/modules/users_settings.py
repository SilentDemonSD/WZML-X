...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        duopt = config_dict.get('DEFAULT_UPLOAD') if (val:=user_dict.get('du_opt')) == '' else val
        ...
        text = BotTheme('UNIVERSAL', NAME=name, YT=escape(ytopt), DT=f"{dailytas} / {dailytl}", LAST_USED=lastused, BOT_PM=bot_pm, MEDIAINFO=mediainfo, SAVE_MODE=save_mode, USESS=u_sess, DT=escape(duopt))
        ...
        if key == 'rcc':
            ...
            set_exist = config_dict.get('DEFAULT_UPLOAD') if (val:=user_dict.get('du_opt')) == '' else val
