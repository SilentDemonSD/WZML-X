...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. Timeout: 60 sec'],
            ...
            'split_size': ['Leech Splits Size is the size to split the Leeched File before uploading', f'Send Leech split size in any comfortable size, like 2Gb, 500MB or 1.46gB. <b>PREMIUM BOT:</b> {IS_PREMIUM_USER}. \n<b>Timeout:</b> 60 sec'],
            'ddl_servers': ['DDL Servers which uploads your File to their Specific Hosting', ''],
            'user_tds': [f'UserTD helps to Upload files via Bot to your Custom Drive Destination via Global SA mail\n\n➲ <b>SA Mail :</b> {config_dict["USER_TD_SA"]}', 'Send User TD details for Use while Mirror/Clone\n<b>Format:</b> name id or link index(optional)\nname2 link2 or id2 index(optional)\n\n<b>NOTE:</b> Drive ID must be valid, then only it will accept, Names can have spaces, All UserTD are updated on every set, To Delete UserTD, give Names separated by each line'],
