...
from bot.helper.ext_utils.bot_utils import is_paid, is_sudo, get_readable_file_size, is_url
...
@bot.on_message(filters.command(BotCommands.ScrapeCommand) & (CustomFilters.authorized_chat | CustomFilters.authorized_user))
async def scrapper(c: Client, message: Message):
    user_id_ = message.from_user.id
    if config_dict['PAID_SERVICE'] is True:
        if user_id_ != OWNER_ID and not is_sudo(user_id_) and not is_paid(user_id_):
            await sendMessage(f"Buy Paid Service to Use this Scrape Feature.", c, message)
            return
    link = None
    links = message.text.split(' ', 1)
    if len(links) == 2:
        if not is_url(links[1]) and links[1].startswith('gdbot:'):
            searchQuery = links[1].split(':', 1)
            res = rget(f"https://gdbot.xyz/search?q={quote(searchQuery[1])}").text
            soup = BeautifulSoup(res, 'html.parser')
            tsear = soup.select("li")
            msg = ''
            for ss in tsear[4:]:
                b = ss.select("a[href*='gdbot.xyz']")[0]
                msg += f'\n\n{b.text}\n'
                msg += f'{ss.select("span")[0].text}'
                resp = rget(b['href']).text
                nsoup = BeautifulSoup(resp, 'html.parser')
                gdtotL = nsoup.select("a[href*='gdtot']")[0]['href']
                msg += f'\n{gdtotL}'
    ...
    if message.reply_to_message:
        ...
        if len(link) == 2:
            ...
            help_msg += f"\n<code>/{BotCommands.ScrapeCommand[0]}" + \
                " {link}" + "</code>\n"
    ...
    try:
        link = rematch(
            r"^(?i)\b((?:https?://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'\".,<>?«»“”‘’]))", link)[0]
    except TypeError:
        return await sendMessage('Not a Valid Link.', c, message)
    links = []
    if "sharespark" in link:
                ...
                if str(next_s).strip():
                    ...
                    for s in List:
                        ...
                        if rematch(r'https?://.+\.gdtot\.\S+', ns):
                            ...
                            nxt = resub(
                                r'\(|\)|(https?://pastetot\.\S+)', '', next_s)
                            gd_txt += f"\n<code>{nxt}</code>\n{ns}\n"
            if len(gd_txt) > 4000:
                await sendMessage(gd_txt, c, message)
                gd_txt = ""
        if gd_txt != "":
            await sendMessage(gd_txt, c, message)
        ...
        for o in links:
            ...
            prsd += f'{title}\n{o}\n\n'
            if len(prsd) > 4000:
                await sendMessage(prsd, c, message)
                prsd = ""
        if prsd != "":
            await sendMessage(prsd, c, message)
        ...
        try:
            ...
            await editMessage("Please provide the correct episode link of animekaizoku", sent)
            return
        try:
            post_id = POST_ID_REGEX.search(website_html).group(
                0).split(":")[1].split('"')[1]
            payload_data_matches = DDL_REGEX.finditer(website_html)
        except:
            await editMessage("Something Went Wrong !!", sent)
            return

        for match in payload_data_matches:
            payload_data = match.group(0).split(
                "DDL(")[1].replace(")", "").split(",")
            payload = {
                "action": "DDL",
                "post_id": post_id,
            ...
            response = rpost("https://animekaizoku.com/wp-admin/admin-ajax.php", headers={
                             "x-requested-with": "XMLHttpRequest", "referer": "https://animekaizoku.com"}, data=payload)
        ...
        for key in main_dict:
            ...
            if bool(dict_data) == 0:
                ...
                for y in dict_data:
                    ...
                    await editMessage(gd_txt, sent)
                    if len(gd_txt) > 3500:
                        sent = await sendMessage('Running More Scrape ...', c, message)
                        gd_txt = ""
                gd_txt += "\n"
        ...
        for txt in links:
            await sendMessage(txt, c, message)


def looper(dict_key, click):
    payload_data = DDL_REGEX.search(click).group(
        0).split("DDL(")[1].replace(")", "").split(",")
    data = {
        "action": "DDL",
        "post_id": post_id,
        "div_id": payload_data[0].strip(),
    ...
    data["num"] = new_num
    response = rpost("https://animekaizoku.com/wp-admin/admin-ajax.php", headers={
                     "x-requested-with": "XMLHttpRequest", "referer": "https://animekaizoku.com"}, data=data)
...
def ouo_parse(dict_key, button, loop_soup):
    try:
        ouo_encrypt = recompile(
            r"openInNewTab\(([^),]+\)"")").search(str(loop_soup)).group(0).strip().split('"')[1]
        ...
        data_dict[dict_key].append(
            [button.text.strip(), decrypted_link.strip()])
    except:
            ...
            if files_type == "application/vnd.google-apps.folder":
                ...
                if not folder_mode:
                    ...
                    data, error = indexScrape(
                        {"page_token": next_page_token, "page_index": 0}, direct_download_link, auth)
                    result.extend(["---------------------------------------------\n\n"] + data + [
                                  "---------------------------------------------\n\n"])
            else:
                filNo += 1
