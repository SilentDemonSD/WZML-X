...
from bot import bot, config_dict, user_data, DATABASE_URL, tgBotMaxFileSize, DRIVES_IDS, DRIVES_NAMES, INDEX_URLS, aria2, GLOBAL_EXTENSION_FILTER, LOGGER, status_reply_dict_lock, Interval, aria2_options, aria2c_global, download_dict, qbit_options, get_client, CATEGORY_NAMES, CATEGORY_IDS, CATEGORY_INDEX, main_loop
...
from bot.helper.ext_utils.bot_utils import setInterval, update_user_ldata
...
async def load_config():
    ...
    DEF_THUMB = environ.get('DEF_THUMB', '')
    if len(DEF_THUMB) == 0:
        DEF_THUMB = ''
    ...
    TORRENT_DIRECT_LIMIT = '' if len(
        TORRENT_DIRECT_LIMIT) == 0 else float(TORRENT_DIRECT_LIMIT)
    ...
    ZIP_UNZIP_LIMIT = '' if len(
        ZIP_UNZIP_LIMIT) == 0 else float(ZIP_UNZIP_LIMIT)

    TOTAL_TASKS_LIMIT = environ.get('TOTAL_TASKS_LIMIT', '')
    TOTAL_TASKS_LIMIT = '' if len(
        TOTAL_TASKS_LIMIT) == 0 else int(TOTAL_TASKS_LIMIT)

    USER_TASKS_LIMIT = environ.get('USER_TASKS_LIMIT', '')
    USER_TASKS_LIMIT = '' if len(
        USER_TASKS_LIMIT) == 0 else int(USER_TASKS_LIMIT)
    ...
    DAILY_MIRROR_LIMIT = '' if len(
        DAILY_MIRROR_LIMIT) == 0 else int(DAILY_MIRROR_LIMIT)

    DAILY_LEECH_LIMIT = environ.get('DAILY_LEECH_LIMIT', '')
    DAILY_LEECH_LIMIT = '' if len(
        DAILY_LEECH_LIMIT) == 0 else int(DAILY_LEECH_LIMIT)
    ...
    QB_LEECH_ENABLED = environ.get('QB_LEECH_ENABLED', '')
    QB_LEECH_ENABLED = QB_LEECH_ENABLED.lower() == 'true'

    LEECH_ENABLED = environ.get('LEECH_ENABLED', '')
    LEECH_ENABLED = LEECH_ENABLED.lower() == 'true'

    YTDL_ENABLED = environ.get('YTDL_ENABLED', '')
    YTDL_ENABLED = YTDL_ENABLED.lower() == 'true'
    ...
    if len(BASE_URL) == 0:
        ...
        srun(["pkill", "-9", "-f", "gunicorn"])
        Popen(
            f"gunicorn web.wserver:app --bind 0.0.0.0:{SERVER_PORT}", shell=True)
    ...
    config_dict.update({'AS_DOCUMENT': AS_DOCUMENT,
                        ...
                        'QB_LEECH_ENABLED': QB_LEECH_ENABLED,
                        'QUEUE_ALL': QUEUE_ALL,
...
def get_buttons(key=None, edit_type=None, user_id=None):
    global userdb_ids
    buttons = ButtonMaker()
    if key is None:
        buttons.sbutton('Config Variables', "botset var")
        buttons.sbutton('UserDB Settings', "botset udb")
        ...
        for x in range(0, len(config_dict)-1, 10):
            buttons.sbutton(
                int(x/10), f"botset start var {x}", position='footer')
        msg = f'Config Variables | Page: {int(START/10)} | State: {STATE}'
    elif key == 'udb':
        if len(userdb_ids) != 0:
            for k in userdb_ids[START:10+START]:
                buttons.sbutton(k, f"botset editudb {k}")
            for x in range(0, len(userdb_ids)-1, 10):
                buttons.sbutton(
                    int(x/10), f"botset start udb {x}", position='footer')
            buttons.sbutton(f'Delete All', 'botset resetudb cnf')
        buttons.sbutton('Add New UserID', "botset editudb newkey")
        buttons.sbutton('Back', "botset back")
        buttons.sbutton('Close', "botset close")
        msg = f'UserDB Settings | Page: {int(START/10)}'
    elif key == 'private':
        buttons.sbutton('Back', "botset back")
        buttons.sbutton('Close', "botset close")
        ...
        for x in range(0, len(aria2_options)-1, 10):
            buttons.sbutton(
                int(x/10), f"botset start aria {x}", position='footer')
        ...
        for x in range(0, len(qbit_options)-1, 10):
            buttons.sbutton(
                int(x/10), f"botset start qbit {x}", position='footer')
...
async def update_buttons(message, key=None, edit_type=None, uid=None):
    msg, button = get_buttons(key, edit_type, uid)
    await editMessage(msg, message, button)


async def edit_variable(c: Client, message: Message, omsg, key):
...
async def edit_userdb(c: Client, message: Message, omsg, key, uid):
    global userdb_ids
    handler_dict[omsg.chat.id] = False
    value = message.text
    if key == 'newval' and uid:
        key, value = [x.strip() for x in value.split(':', 1)]
    elif value.lower() == 'true' and uid:
        value = True
    elif value.lower() == 'false' and uid:
        value = False
    if uid:
        user_data[uid][key] = value
        update_user_ldata(uid, key, value)
        await update_buttons(omsg, uid, 'udbunits')
    else:
        try:
            await bot.get_chat(value)
        except BadRequest:
            await sendMessage("Not a UserID, Try Again", c, message)
            await update_buttons(omsg, 'udb')
            return
        userdb_ids.append(value)
        await update_buttons(omsg, 'udb')
    await message.delete()
    if DATABASE_URL and uid:
        DbManger().update_user_data(uid)


async def edit_aria(c: Client, message: Message, omsg, key):
    handler_dict[omsg.chat.id] = False
    value = message.text
    if key == 'newkey':
        key, value = [x.strip() for x in value.split(':', 1)]
...
async def edit_qbit(c: Client, message: Message, omsg, key):
    handler_dict[omsg.chat.id] = False
    value = message.text
    if value.lower() == 'true':
        value = True
...
async def update_private_file(c: Client, message: Message, omsg):
    handler_dict[omsg.chat.id] = False
...
@bot.on_callback_query(filters.regex(r'^botset'))
async def edit_bot_settings(c: Client, query: CallbackQuery):
    ...
    if not CustomFilters.owner_query(user_id):
        ...
        if key is None:
            ...
            await update_buttons(message, data[3], key)
            return
        await update_buttons(message, key)
    elif data[1] in ['var', 'udb', 'aria', 'qbit']:
        await query.answer()
        ...
        if data[2] in ['SEARCH_PLUGINS', 'SEARCH_API_LINK']:
            ...
            start_from_queued()
    elif data[1] == 'resetudb':
        global userdb_ids
        cnf = data[2] if len(data) >= 3 else None
        if cnf and cnf == 'cnf':
            await query.answer()
            await update_buttons(message, 'dummp', data[1])
            return
        elif cnf and cnf == 'rem':
            await query.answer()
            userdb_ids.remove(data[3])
        elif cnf and cnf == 'del':
            for id in userdb_ids:
                user_data[int(id)] = {}
                if DATABASE_URL:
                    DbManger().update_user_data(int(id))
            await query.answer(f"All {len(userdb_ids)} User ID(s) Successfully Deleted", show_alert=True)
            userdb_ids.clear()
        await update_buttons(message, 'udb')
    elif data[1] == 'deleteudb':
        handler_dict[message.chat.id] = False
        user_data[int(data[3])].pop(data[2])
        await query.answer(f"{data[2]} Successfully Deleted for {data[3]}", show_alert=True)
        await update_buttons(message, data[3], 'udbunits')
        if DATABASE_URL:
        ...
        if aria2_defaults[data[2]] == aria2_options[data[2]]:
            await query.answer(text='Value already same as you added in aria.sh!')
            return
        await query.answer()
        ...
        try:
            m = await c.listen.Message((filters.document | filters.text) & filters.chat(message.chat.id), id=filters.user(user_id), timeout = 60)
        except AsyncTimeoutError:
            m = None
        if m:
            await update_private_file(c, m, message)
        else:
        ...
        if data[2] in boolVars:
            return
        handler_dict[message.chat.id] = True
        try:
            m = await c.listen.Message(filters.text & filters.chat(message.chat.id), id=filters.user(user_id), timeout = 60)
        except AsyncTimeoutError:
            m = None
        if m:
            await edit_variable(c, m, message, data[2])
        else:
            await update_buttons(message, 'var')
        ...
        if len(str(value)) > 200:
            ...
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(f'{value}')
            await sendFile(c, message, filename)
            return
        elif value == '':
            value = None
        await query.answer(text=f'{value}', show_alert=True)
    elif data[1] == 'editudb' and STATE == 'view' and len(data) == 4 and data[2] != 'newkey' and data[2] != 'newval':
        value = user_data[int(data[3])][data[2]]
        if len(str(value)) > 200:
            await query.answer()
            filename = f"{data[2]}.txt"
            with open(filename, 'w', encoding='utf-8') as f:
            ...
            value = 'Empty String'
        await query.answer(text=f'{value}', show_alert=True)
    elif data[1] == 'editudb' and data[2]:
        await query.answer()
        uid = int(data[3]) if len(data) == 4 else None
        if data[2] != "newkey" and not uid:
            await update_buttons(message, data[2], 'udbunits')
            return
        if handler_dict.get(message.chat.id):
            handler_dict[message.chat.id] = False
            await asleep(0.5)
        start_time = time()
        handler_dict[message.chat.id] = True
        await update_buttons(message, data[2], data[1], uid)
        try:
            m = await c.listen.Message(filters.text & filters.chat(message.chat.id), id=filters.user(user_id), timeout = 60)
        except AsyncTimeoutError:
            m = None
        if m:
            await edit_userdb(c, m, message, data[2], uid)
        else:
            await update_buttons(message, 'udb')
        ...
        try:
            ...
            m = None
        if m:
            await edit_aria(c, m, message, data[2])
        else:
            await update_buttons(message, 'aria')
        ...
        try:
            ...
            m = None
        if m:
            await edit_qbit(c, m, message, data[2])
        else:
            await update_buttons(message, 'qbit')
        ...
        await query.answer()
        if START2 != int(data[3]):
            globals()['START2'] = int(data[3])
            await update_buttons(message, data[4], data[2])
    elif data[1] == 'push':
        await query.answer()
...
@bot.on_message(filters.command(BotCommands.BotSetCommand) & (CustomFilters.owner_filter | CustomFilters.sudo_user))
async def bot_settings(c: Client, message: Message):
    msg, button = get_buttons()
    globals()['START'] = 0
    await sendMessage(msg, c, message, button)
