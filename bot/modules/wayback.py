...
@bot.on_message(filters.command(BotCommands.WayBackCommand) & (authfilter | CustomFilters.authorized_user))
async def wayback(c: Client, message: Message):
    link = None
    ...
    except TypeError: return await sendMessage('Not a valid link for wayback.', c, message)
    sent = await sendMessage('Running WayBack. Wait about 20 secs.', c, message)
    retLink = saveWebPage(link)
    if not retLink: return await editMessage('Cannot archieved. Try again later.', sent)
    await editMessage(f'Saved webpage: {short_url(retLink, message.from_user.id)}', sent)
