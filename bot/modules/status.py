...
async def status_pages(_, query):
    ...
    if data[1] == 'ref':
        ...
        if user_id in refresh_status and time() - refresh_status[user_id] < 7:
            return await query.answer(f'Already Refreshed! Try after {get_readable_time(7 - (time() - refresh_status[user_id]))}', show_alert=True)
        refresh_status[user_id] = time()
        await editMessage(query.message, f"<b>{await user_info(user_id).mention(html=True)}</b>, <i>Refreshing Status...</i>")
