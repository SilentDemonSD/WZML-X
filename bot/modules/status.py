...
async def status_pages(_, query):
    ...
    if data[1] == 'ref':
        ...
        if user_id in (refresh_status := bot_cache['status_refresh']) and (curr := (time() - refresh_status[user_id])) <= 5:
            return await query.answer(f'Already Refreshed! Try after {5 - curr}s')
        else:
