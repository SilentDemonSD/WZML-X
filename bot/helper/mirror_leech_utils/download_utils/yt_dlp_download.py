    ...
    def debug(self, msg):
        ...
        if not self._obj.is_playlist:
            if match := re_search(
                ...
                self._listener.self._listener.name = newname
...
class YoutubeDLHelper:
    ...
    def extractMetaData(self):
        ...
        with YoutubeDL(self.opts) as ydl:
            ...
            if 'entries' in result:
                self._listener.name = self._listener.name
    ...
    async def add_download(self, path, qual, playlist, options):
        ...
        if self.is_playlist:
            ...
            self.opts['outtmpl'] = {'default': f"{path}/{base_name}/{self._listener.name}",
                                    'thumbnail': f"{path}/yt-dlp-thumb/{base_name}.%(ext)s"}
        else:
            self.opts['outtmpl'] = {'default': f"{path}/{self._listener.name}",
                                    'thumbnail': f"{path}/yt-dlp-thumb/{base_name}.%(ext)s"}
        ...
        if self._ext in ['.mp3', '.mkv', '.mka', '.ogg', '.opus', '.flac', '.m4a', '.mp4', '.mov', 'm4v']:
            self.opts['postprocessors'].append(
                {'already_have_thumbnail': self._listener.isLeech, 'key': 'EmbedThumbnail'})
        ...
        if limit_exceeded := await limit_checker(self._listener.size, self._listener, isYtdlp=True, isPlayList=self.playlist_count):
