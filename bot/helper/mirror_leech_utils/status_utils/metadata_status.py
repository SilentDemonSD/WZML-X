...
class MetadataStatus:
    ...
    def speed(self):
        try:
            return f"{get_readable_file_size(self._obj.speed_raw)}/s" if hasattr(self._obj, 'speed_raw') and self._obj.speed_raw else "0 B/s"
        except Exception:
            return "0 B/s"

    def processed_bytes(self):
        try:
            return get_readable_file_size(self._obj.processed_bytes) if hasattr(self._obj, 'processed_bytes') and self._obj.processed_bytes else "0 B"
        except Exception:
            return "0 B"

    def progress(self):
        try:
            if hasattr(self._obj, 'progress_raw') and self._obj.progress_raw is not None:
    ...
    def eta(self):
        try:
            if hasattr(self._obj, 'eta_raw') and self._obj.eta_raw is not None and self._obj.eta_raw > 0:
