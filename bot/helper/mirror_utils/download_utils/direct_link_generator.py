...
def zippy_share(url: str) -> str:
    ...
    dl_url = base_url+"/"+uri1+"/"+int(mtk)+"/"+uri2
    return dl_url
...
def pixeldrain(url: str) -> str:
    ...
    if url.split("/")[-2] == "l":
        ...
        dl_link = "https://pixeldrain.com/api/list/"+file_id+"/zip"
    else:
        info_link = f"https://pixeldrain.com/api/file/{file_id}/info"
        dl_link = "https://pixeldrain.com/api/file/"+file_id
...
# async def prun(playwright: Playwright, link: str) -> str:
...
#     await expect(firstbtn).to_be_visible()
#     await firstbtn.click()
#     await asleep(6)

#     secondBtn = page.get_by_role("button", name="Download Now")
#     await expect(secondBtn).to_be_visible()
...
# async def filepress(link: str) -> str:
#     async with async_playwright() as playwright:
#         flink = await prun(playwright, link)
#         return flink
