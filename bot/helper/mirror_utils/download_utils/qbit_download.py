...
async def add_qb_torrent(link: str, path: str, listener, ratio: float, seed_time: int) -> Tuple[str, str]:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client_:
            ...
            if response.status_code == 200:
                torrent_file = await aioopen(link, mode='rb')
                torrent_data = await torrent_file.read()
                torrent_file.close()
                decoded_dict = bdecode(torrent_data)
                hash_ = sha1(bencode(decoded_dict['info'])).hexdigest()
            else:
                hash_ = link.split('magnet:?xt=urn:btih:')[-1]
