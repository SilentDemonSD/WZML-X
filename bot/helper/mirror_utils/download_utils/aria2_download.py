...
from bot import aria2, download_dict_lock, download_dict, LOGGER, config_dict, user_data, aria2_options, aria2c_global, OWNER_ID, main_loop
    ...
    if download.is_metadata:
        ...
        if dl := getDownloadByGid(gid):
            ...
            if listener.select:
                ...
                meta = main_loop.create_task(sendMessage(metamsg, listener.bot, listener.message))
                while True:
                    if download.is_removed or download.followed_by_ids:
                        main_loop.create_task(deleteMessage(listener.bot, meta))
                        break
                    download = download.live
    else:
        LOGGER.info(f'onDownloadStarted: {download.name} - Gid: {gid}')
    future = run_coroutine_threadsafe(__onLimitsVerify(api, gid), main_loop)
    future.result()

async def __onLimitsVerify(api, gid):
    try:
        ...
        if any([STOP_DUPLICATE, TORRENT_DIRECT_LIMIT, ZIP_UNZIP_LIMIT, LEECH_LIMIT, STORAGE_THRESHOLD, DAILY_MIRROR_LIMIT, DAILY_LEECH_LIMIT]):
            ...
            if STOP_DUPLICATE and not dl.listener().isLeech and IS_USRTD == False:
                ...
                if sname is not None:
                    gdHelper = GoogleDriveHelper(user_id=user_id)
                    smsg, button = await gdHelper.drive_list(sname, True)
                    if smsg:
                        await listener.onDownloadError("File/Folder is already available in Drive.")
                        api.remove([download], force=True, files=True, clean=True)
                        tegr, html, tgdi = userlistype(user_id)
                        if html:
                            return await sendFile(listener.bot, listener.message, button, f"Here are the search results:\n\n{smsg}")
                        elif tegr:
                            return await sendMessage("Here are the search results:", listener.bot, listener.message, button)
                        else: return await sendMessage(smsg, listener.bot, listener.message, button)
            ...
            if any([ZIP_UNZIP_LIMIT, LEECH_LIMIT, TORRENT_DIRECT_LIMIT, STORAGE_THRESHOLD]) and user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id):
                ...
                if STORAGE_THRESHOLD:
                    ...
                    if not acpt:
                        ...
                        return api.remove([download], force=True, files=True, clean=True)
                ...
                if limit:
                    ...
                    if size > limit * 1024**3:
                        await listener.onDownloadError(f'{mssg}.\nYour File/Folder size is {get_readable_file_size(size)}')
                        return api.remove([download], force=True, files=True, clean=True)
            if DAILY_MIRROR_LIMIT and not listener.isLeech and user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id) and (size >= (DAILY_MIRROR_LIMIT - getdailytasks(user_id, check_mirror=True)) or DAILY_MIRROR_LIMIT <= getdailytasks(user_id, check_mirror=True)):
                ...
                if config_dict['PAID_SERVICE'] is True:
                    mssg += f'\n#Buy Paid Service'
                await listener.onDownloadError(mssg)
                return api.remove([download], force=True, files=True, clean=True)
        ...
        LOGGER.error(f"{format_exc()}\n\nonDownloadStart: {gid} Stop Duplicate and Size Check didn't Pass")
    ...
    if download.followed_by_ids:
        ...
        if dl := getDownloadByGid(new_gid):
            ...
            if config_dict['BASE_URL'] and listener.select:
                ...
                main_loop.create_task(sendMessage(msg, listener.bot, listener.message, SBUTTONS))
    elif download.is_torrent:
        if dl := getDownloadByGid(gid):
            if hasattr(dl, 'listener') and dl.seeding:
                LOGGER.info(f"Cancelling Seed: {download.name} onDownloadComplete")
                future = run_coroutine_threadsafe(dl.listener().onUploadError(f"Seeding stopped with Ratio: {dl.ratio()} and Time: {dl.seeding_time()}"), main_loop)
                future.result()
                api.remove([download], force=True, files=True, clean=True)
    else:
        LOGGER.info(f"onDownloadComplete: {download.name} - Gid: {gid}")
        if dl := getDownloadByGid(gid):
            future = run_coroutine_threadsafe(dl.listener().onDownloadComplete(), main_loop)
            future.result()
            api.remove([download], force=True, files=True, clean=True)
...
def __onBtDownloadComplete(api, gid):
    ...
    if dl := getDownloadByGid(gid):
        ...
        future = run_coroutine_threadsafe(listener.onDownloadComplete(), main_loop)
        future.result()
        download = download.live
        if listener.seed:
            if download.is_complete:
                if dl := getDownloadByGid(gid):
                    ...
                    future = run_coroutine_threadsafe(listener.onUploadError(f"Seeding stopped with Ratio: {dl.ratio()} and Time: {dl.seeding_time()}"), main_loop)
                    future.result()
                    api.remove([download], force=True, files=True, clean=True)
            else:
                with (yield from download_dict_lock):
                ...
                main_loop.create_task(update_all_messages())
        else:
            api.remove([download], force=True, files=True, clean=True)
...
def __onDownloadStopped(api, gid):
    sleep(6)
    if dl := getDownloadByGid(gid):
        run_coroutine_threadsafe(dl.listener().onDownloadError('Dead torrent!'), main_loop)
...
def __onDownloadError(api, gid):
    ...
    try:
        ...
        pass
    if dl := getDownloadByGid(gid):
        run_coroutine_threadsafe(dl.listener().onDownloadError(error), main_loop)
...
async def add_aria2c_download(link: str, path, listener, filename, auth, ratio, seed_time):
    ...
    if ospath.exists(link) and link.endswith('.torrent'):
        download = aria2.add_torrent(link, options=args)
