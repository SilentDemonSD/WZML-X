...
from bot import download_dict, download_dict_lock, LOGGER, user_data, config_dict, OWNER_ID, non_queued_dl, non_queued_up, queued_dl, queue_dict_lock, main_loop
...
async def add_gd_download(link, path, listener, newname, is_gdtot, is_udrive, is_sharer, is_sharedrive, is_filepress, is_unified, from_queue=False):
    res, size, name, files = await main_loop.create_task(GoogleDriveHelper().helper(link))
    if res != "":
        return await sendMessage(res, listener.bot, listener.message)
    if newname:
        name = newname
    ...
    if config_dict['STOP_DUPLICATE'] and not listener.isLeech and IS_USRTD == False:
        ...
        if gname is not None:
            gmsg, button = await main_loop.create_task(GoogleDriveHelper(user_id=user_id).drive_list(gname, True))
            if gmsg:
                ...
                if tegr:
                    ...
                    return await sendFile(listener.bot, listener.message, button, f"File/Folder is already available in Drive. Here are the search results:\n\n{gmsg}")
                else: return await sendMessage(gmsg, listener.bot, listener.message, button)
        ...
        if STORAGE_THRESHOLD:
            ...
            if not acpt:
                ...
                if config_dict['PAID_SERVICE'] is True:
                    msg += f'\n#Buy Paid Service'
                return await sendMessage(msg, listener.bot, listener.message)
    ...
    if DAILY_MIRROR_LIMIT and not listener.isLeech and user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id) and (size >= (DAILY_MIRROR_LIMIT - getdailytasks(user_id, check_mirror=True)) or DAILY_MIRROR_LIMIT <= getdailytasks(user_id, check_mirror=True)):
        ...
        if config_dict['PAID_SERVICE'] is True:
            mssg += f'\n#Buy Paid Service'
        return await sendMessage(mssg, listener.bot, listener.message)
    ...
    if all_limit or dl_limit:
        ...
        async with queue_dict_lock:
            ...
            if (all_limit and dl + up >= all_limit and (not dl_limit or dl >= dl_limit)) or (dl_limit and dl >= dl_limit):
                added_to_queue = True
                queued_dl[listener.uid] = ['gd', link, path, listener, newname, is_gdtot, is_udrive, is_sharer, is_sharedrive, is_filepress, is_unified]
        if added_to_queue:
    ...
    await main_loop.create_task(drive.download(link))
