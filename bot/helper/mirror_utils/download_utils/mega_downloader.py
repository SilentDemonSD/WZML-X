...
from bot import LOGGER, download_dict, download_dict_lock, config_dict, \
    user_data, OWNER_ID, non_queued_dl, non_queued_up, queued_dl, queue_dict_lock, main_loop
...
class MegaAppListener(MegaListener):
    ...
    def onRequestTemporaryError(self, api, request, error: MegaError):
        ...
        if not self.is_cancelled:
            ...
            main_loop.create_task(self.listener.onDownloadError(
    ...
    def onTransferFinish(self, api: MegaApi, transfer: MegaTransfer, error):
        try:
            if self.is_cancelled:
                ...
                main_loop.create_task(self.listener.onDownloadComplete())
                self.continue_event.set()
...
async def add_mega_download(mega_link, path, listener, name, from_queue=False):
    ...
    await main_loop.run_in_executor(None, api.addListener, mega_listener)
    user_id = listener.message.from_user.id
    user_dict = user_data.get(user_id, False)
    if MEGA_EMAIL_ID and MEGA_PASSWORD:
        await main_loop.run_in_executor(None, executor.do, api.login, (MEGA_EMAIL_ID, MEGA_PASSWORD))
        # await main_loop.run_in_executor(executor, executor.do, api.login, (MEGA_EMAIL_ID, MEGA_PASSWORD))
    if get_mega_link_type(mega_link) == "file":
        await main_loop.run_in_executor(None, executor.do, api.getPublicNode, (mega_link,))
        node = mega_listener.public_node
    else:
        folder_api = MegaApi(MEGA_API_KEY, None, None, 'mltb')
        await main_loop.run_in_executor(None, folder_api.addListener, mega_listener)
        await main_loop.run_in_executor(None, executor.do, folder_api.loginToFolder, (mega_link,))
        node = await main_loop.run_in_executor(None, folder_api.authorizeNode, mega_listener.node)
    if mega_listener.error is not None:
        await sendMessage(str(mega_listener.error), listener.bot, listener.message)
        await main_loop.run_in_executor(None, api.removeListener, mega_listener)
        if folder_api is not None:
            await main_loop.run_in_executor(None, folder_api.removeListener, mega_listener)
        return
    ...
    if config_dict['STOP_DUPLICATE'] and not listener.isLeech and IS_USRTD == False:
        ...
        if mname is not None:
            # smsg, button = main_loive_list(mname, True))
            smsg, button = await GoogleDriveHelper(user_id=user_id).drive_list(mname, True)
            if smsg:
                tegr, html, tgdi = userlistype(user_id)
                if tegr:
                    await sendMessage("File/Folder is already available in Drive.\nHere are the search results:", listener.bot, listener.message, button)
                elif html:
                    await sendFile(listener.bot, listener.message, button,
    ...
    DAILY_MIRROR_LIMIT = config_dict['DAILY_MIRROR_LIMIT'] * \
        1024**3 if config_dict['DAILY_MIRROR_LIMIT'] else config_dict['DAILY_MIRROR_LIMIT']
    DAILY_LEECH_LIMIT = config_dict['DAILY_LEECH_LIMIT'] * \
        1024**3 if config_dict['DAILY_LEECH_LIMIT'] else config_dict['DAILY_LEECH_LIMIT']

    size = await main_loop.run_in_executor(None, api.getSize, node)
    if any([STORAGE_THRESHOLD, ZIP_UNZIP_LIMIT, MEGA_LIMIT, LEECH_LIMIT]) and user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id):
        ...
        if STORAGE_THRESHOLD is not None:
            ...
            if not acpt:
                ...
                if config_dict['PAID_SERVICE'] is True:
                    msg += f'\n#Buy Paid Service'
                await sendMessage(msg, listener.bot, listener.message)
                return
        limit = None
        ...
        if limit is not None:
            ...
            if size > limit * 1024**3:
                await sendMessage(msg3, listener.bot, listener.message)
                return
    if DAILY_MIRROR_LIMIT and not listener.isLeech and user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id) and (size >= (DAILY_MIRROR_LIMIT - getdailytasks(user_id, check_mirror=True)) or DAILY_MIRROR_LIMIT <= getdailytasks(user_id, check_mirror=True)):
        ...
        if config_dict['PAID_SERVICE'] is True:
            mssg += f'\n#Buy Paid Service'
        await sendMessage(mssg, listener.bot, listener.message)
        return
    ...
    makedirs(path)
    await main_loop.run_in_executor(None, mega_listener.setValues, mname, size, gid)
    if not from_queue:
        await listener.onDownloadStart()
        await sendStatusMessage(
            listener.message, listener.bot)
    ...
    await main_loop.run_in_executor(None, executor.do, api.startDownload, (node, path, name, None, False, None))
    await main_loop.run_in_executor(None, api.removeListener, mega_listener)
    if folder_api is not None:
        await main_loop.run_in_executor(None, folder_api.removeListener, mega_listener)
