...
from bot import OWNER_ID, config_dict, download_dict, download_dict_lock, non_queued_dl, non_queued_up, queued_dl, queue_dict_lock, main_loop
...
class YoutubeDLHelper:
    ...
    async def __onDownloadStart(self, from_queue):
        ...
        if not from_queue:
            ...
            await sendStatusMessage(self.listener.message, self.listener.bot)
    ...
    def extractMetaData(self, link, name, args, get_info=False):
        ...
        with YoutubeDL(self.opts) as ydl:
            try:
                ...
                if get_info:
                    raise e
                return main_loop.create_task(self.__onDownloadError(str(e)))
        if self.is_playlist:
    ...
    async def __download(self, link, dpath):
        try:
            with YoutubeDL(self.opts) as ydl:
                try:
                    ydl.download([link])
    ...
    async def add_download(self, link, dpath, name, qual, playlist, args, from_queue=False):
        if playlist:
        ...
        if config_dict['STOP_DUPLICATE'] and self.name != 'NA' and not self.listener.isLeech:
            ...
            if sname:
                smsg, button = await GoogleDriveHelper(user_id=user_id).drive_list(name, True, True)
                if smsg:
        ...
        if not limit_exceeded and (LEECH_LIMIT:= config_dict['LEECH_LIMIT']) and self.listener.isLeech and user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id):
            ...
            if self.__size > limit:
                ...
                limit_exceeded += f'is {get_readable_file_size(self.__size)}'
        if limit_exceeded:
            return await self.__onDownloadError(limit_exceeded)
        ...
        if DAILY_MIRROR_LIMIT and not self.listener.isLeech and user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id) and (self.__size >= (DAILY_MIRROR_LIMIT - getdailytasks(user_id, check_mirror=True)) or DAILY_MIRROR_LIMIT <= getdailytasks(user_id, check_mirror=True)):
            ...
            if config_dict['PAID_SERVICE'] is True:
                mssg += f'\n#Buy Paid Service'
            return await self.__onDownloadError(mssg)
        ...
        await self.__download(link, dpath)
