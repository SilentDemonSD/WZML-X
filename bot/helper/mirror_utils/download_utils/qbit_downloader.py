...
from bot import download_dict, download_dict_lock, get_client, config_dict, QbInterval, user_data, LOGGER, OWNER_ID, main_loop
...
async def add_qb_torrent(link, path, listener, ratio, seed_time):
    ...
    try:
        if link.startswith('magnet:'):
            ext_hash = await main_loop.run_in_executor(None, __get_hash_magnet, link)
        else:
            ext_hash = await main_loop.run_in_executor(None, __get_hash_file, link)
        if ext_hash is None or len(ext_hash) < 30:
            await sendMessage("Not a torrent! Qbittorrent only for torrents!", listener.bot, listener.message)
            return
        tor_info = client.torrents_info(torrent_hashes=ext_hash)
        if len(tor_info) > 0:
            await sendMessage("This Torrent already added!", listener.bot, listener.message)
            return
        if link.startswith('magnet:'):
            # op =  client.torrents_add(
            #     link, save_path=path, ratio_limit=ratio, seeding_time_limit=seed_time)
            op = await main_loop.run_in_executor(None, partial(client.torrents_add,
                link, save_path=path, ratio_limit=ratio, seeding_time_limit=seed_time))
        else:
            # op = client.torrents_add(torrent_files=[
            #                          link], save_path=path, ratio_limit=ratio, seeding_time_limit=seed_time)
            op = await main_loop.run_in_executor(None, partial(client.torrents_add, torrent_files=[link], save_path=path, ratio_limit=ratio, seeding_time_limit=seed_time))
        await asleep(0.3)
        if op.lower() == "ok.":
            tor_info = client.torrents_info(torrent_hashes=ext_hash)
            if len(tor_info) == 0:
            ...
            await sendMessage("This is an unsupported/invalid link.", listener.bot, listener.message)
            await __remove_torrent(client, ext_hash)
            return
        tor_info = tor_info[0]
        ...
        if config_dict['BASE_URL'] and listener.select:
            if link.startswith('magnet:'):
                ...
                meta = await sendMessage(metamsg, listener.bot, listener.message)
                while True:
                    tor_info = client.torrents_info(torrent_hashes=ext_hash)
                    if len(tor_info) == 0:
                    ...
                    try:
                        ...
                        return await deleteMessage(listener.bot, meta)
            client.torrents_pause(torrent_hashes=ext_hash)
            SBUTTONS = bt_selection_buttons(ext_hash)
            msg = "Your download paused. Choose files then press Done Selecting button to start downloading."
            await sendMessage(msg, listener.bot, listener.message, SBUTTONS)
        else:
            await sendStatusMessage(listener.message, listener.bot)
    except Exception as e:
        await sendMessage(str(e), listener.bot, listener.message)
    finally:
    ...
    try:
        ...
        if not listener.select and not listener.isLeech and IS_USRTD == False:
            ...
            if qbname is not None:
                qbmsg, button = await main_loop.create_task(GoogleDriveHelper(
                    user_id=user_id).drive_list(qbname, True))
                if qbmsg:
                    await __onDownloadError("File/Folder is already available in Drive.", client, tor)
                    tegr, html, tgdi = userlistype(user_id)
                    if tegr:
                        await sendMessage("Here are the search results:", listener.bot, listener.message, button)
                    elif html:
                        await sendFile(listener.bot, listener.message, button, f"Here are the search results:\n\n{qbmsg}")
                    else:
                        await sendMessage(qbmsg, listener.bot, listener.message, button)
                    return
    except:
        pass


async def __check_limits(client, tor):
    download = getDownloadByGid(tor.hash[:12])
    listener = download.listener()
    size = tor.size
        ...
        if STORAGE_THRESHOLD is not None:
            ...
            if not acpt:
                ...
                if config_dict['PAID_SERVICE'] is True:
                    msg += f'\n#Buy Paid Service'
                await __onDownloadError(msg, client, tor)
                return
                limit = None
    ...
    if DAILY_MIRROR_LIMIT and not listener.isLeech and user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id) and (size >= (DAILY_MIRROR_LIMIT - getdailytasks(user_id, check_mirror=True)) or DAILY_MIRROR_LIMIT <= getdailytasks(user_id, check_mirror=True)):
        ...
        if config_dict['PAID_SERVICE'] is True:
            mssg += f'\n#Buy Paid Service'
        await __onDownloadError(mssg, client, tor)
            ...
            for tor_info in client.torrents_info():
                if tor_info.state == "metaDL":
                    ...
                    if config_dict['STOP_DUPLICATE'] and tor_info.hash not in STOP_DUP_CHECK:
                        ...
                        await __stop_duplicate(client, tor_info)
                    if (config_dict['TORRENT_DIRECT_LIMIT'] or config_dict['ZIP_UNZIP_LIMIT'] or config_dict['LEECH_LIMIT'] or config_dict['STORAGE_THRESHOLD'] or config_dict['DAILY_MIRROR_LIMIT'] or config_dict['DAILY_LEECH_LIMIT']) and tor_info.hash not in LIMITS_CHECK:
                        ...
                        await __check_limits(client, tor_info)
