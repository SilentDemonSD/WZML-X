...
async def cmd_exec_safe(cmd: List[str]) -> Tuple[str, int]:
    result, err_code = await cmd_exec(cmd)
    if err_code != 0:
        raise Exception(f'Error while executing command: {cmd}\nError: {result[:4000]}')
    return result, err_code

# Function to get rclone stats and size
async def get_rclone_stats_and_size(
        remote: str, rc_path: str, config_path: str) -> Tuple[Dict[str, Any], int]:
    ...
    res1, res2 = await gather(cmd_exec_safe(cmd1), cmd_exec_safe(cmd2))
    return loads(res1[0]), res2[1]

# Function to add rclone download
async def add_rclone_download(rc_path: str, config_path: str, path: str, name: Optional[str], listener):
    remote, rc_path = rc_path.split(':', 1)
    rc_path = rc_path.strip('/')

    rstat, rstat_err_code = get_rclone_stats_and_size(remote, rc_path, config_path)
    rsize, rsize_err_code = get_rclone_stats_and_size(remote, rc_path, config_path)

    if rstat_err_code != rsize_err_code != 0:
        if rstat_err_code != -9:
            err = f'{res1[1]} {res2[1]}'
    ...
    size = rsize['bytes']
    gid = ''.join(random.SystemRandom().choices(ascii_letters + digits, k=12))
