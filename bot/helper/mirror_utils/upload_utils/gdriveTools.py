...
from bot import bot, config_dict, ulist_listener, user_data, DRIVES_NAMES, DRIVES_IDS, INDEX_URLS, GLOBAL_EXTENSION_FILTER, main_loop
...
class GoogleDriveHelper:
    ...
    async def upload(self, file_name: str, u_index, c_index):
        self.__is_uploading = True
        file_path = f"{self.__path}/{file_name}"
        size = await main_loop.run_in_executor(None, get_readable_file_size, self.__size)
        LOGGER.info(f"Uploading File: {file_path}")
        self.__updater = setInterval(self.__update_interval, self._progress)
        GDRIVEID, _ = await main_loop.run_in_executor(None, getGDriveUploadUtils, self.user_id, u_index, c_index)
        try:
            if ospath.isfile(file_path):
                mime_type = await main_loop.run_in_executor(None, get_mime_type, file_path)
                link = await main_loop.run_in_executor(None, self.__upload_file, file_path, file_name, mime_type, GDRIVEID)
                ...
                mime_type = 'Folder'
                dir_id = await main_loop.run_in_executor(None, self.__create_directory, ospath.basename(ospath.abspath(file_name)), GDRIVEID)
                result = await main_loop.run_in_executor(None, self.__upload_dir, file_path, dir_id)
                if result is None:
                    raise Exception('Upload has been manually cancelled!')
            ...
            if self.__is_cancelled and not self.__is_errored:
                if mime_type == 'Folder':
                    ...
                    link = f"https://drive.google.com/folderview?id={dir_id}"
                    await main_loop.run_in_executor(None, self.deletefile, link)
                return
            elif self.__is_errored:
                return
        await self.__listener.onUploadComplete(link, size, self.__total_files, self.__total_folders, mime_type, self.name)
    ...
    async def clone(self, link, u_index, c_index):
        ...
        try:
            meta = await main_loop.run_in_executor(None, self.__getFileMetadata, file_id)
            mime_type = meta.get("mimeType")
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                dir_id = await main_loop.run_in_executor(None, self.__create_directory, meta.get('name'), GDRIVEID)
                await main_loop.run_in_executor(None, self.__cloneFolder, meta.get('name'), meta.get('name'), meta.get('id'), dir_id)
                durl = self.__G_DRIVE_DIR_BASE_DOWNLOAD_URL.format(dir_id)
                if self.__is_cancelled:
                    LOGGER.info("Deleting cloned data from Drive...")
                    await main_loop.run_in_executor(None, self.deletefile, durl)
                ...
                file = await main_loop.run_in_executor(None, self.__copyFile, meta.get('id'), GDRIVEID, meta.get('name'))
                if config_dict['EMOJI_THEME']:
            ...
            if "User rate limit exceeded" in err:
                ...
                token_service = await main_loop.run_in_executor(None, self.__alt_authorize)
                if token_service is not None:
                    self.__service = token_service
                    return await self.clone(link, c_index)
                msg = "File not found."
            else:
                msg = f"Error.\n{err}"
            return msg, ""
    ...
    async def drive_list(self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType=""):
        msg = ""
        ...
        if tegr:
            ...
            buttons = ButtonMaker()
            buttons.buildbutton("🔎 VIEW", f"https://te.legra.ph/{path[0]}")
...
╰ <b>CC :</b> <a href='tg://user?id={self.user_id}'>{usrchat.first_name}</a>'''
    ...
    async def count(self, link):
        ...
        try:
            ...
            mime_type = meta.get('mimeType')
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                await main_loop.run_in_executor(None, self.__gDrive_directory, meta)
                if config_dict['EMOJI_THEME']:
                ...
                if mime_type is None:
                    mime_type = 'File'
                self.__total_files += 1
                await main_loop.run_in_executor(None, self.__gDrive_file, meta)
                if config_dict['EMOJI_THEME']:
                    msg += f'\n<b>├📦 Size: </b>{get_readable_file_size(self.__total_bytes)}'
                    msg += f'\n<b>├♻ Type: </b>{mime_type}'
                else:
    ...
    async def helper(self, link):
        try:
            file_id = await main_loop.run_in_executor(None, self.__getIdFromUrl, link)
        except (KeyError, IndexError):
            msg = "Google Drive ID could not be found in the provided link"
            return msg, "", "", ""
        LOGGER.info(f"File ID: {file_id}")
    ...
    async def download(self, link):
        ...
        try:
            meta = await main_loop.run_in_executor(None, self.__getFileMetadata, file_id)
            if meta.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                await main_loop.run_in_executor(None, self.__download_folder, file_id, self.__path, self.name)
            else:
                makedirs(self.__path)
                await main_loop.run_in_executor(None, self.__download_file, file_id, self.__path, self.name, meta.get('mimeType'))
        except Exception as err:
            if isinstance(err, RetryError):
                LOGGER.info(f"Total Attempts: {err.last_attempt.attempt_number}")
                err = err.last_attempt.exception()
