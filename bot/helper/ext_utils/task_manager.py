...
async def pre_task_check(message):
    ...
    if message.chat.type != message.chat.type.BOT:
        ...
        if Config.BOT_PM or user_dict.get("BOT_PM"):  # or config_dict['SAFE_MODE']:
            ...
            if _msg:
                msg.append(_msg)
    if (uti := Config.USER_TIME_INTERVAL != 0) and (
        ut := await user_interval_check(user_id)
    ):
        msg.append(
            f"┠ <b>Waiting Time</b> → {get_readable_time(ut)}\n┠ <i>User have Time Interval Restrictions for {get_readable_time(uti)}.</i>"
        )
    if ((bmax_tasks := Config.BOT_MAX_TASKS) != 0) and len(await get_specific_tasks("All", False)) >= bmax_tasks:
        msg.append(f"┠ Max Concurrent Bot's Tasks Limit exceeded.\n┠ Bot Tasks Limit : {bmax_tasks} task")
    if ((maxtask := Config.USER_MAX_TASKS) != 0) and len(await get_specific_tasks("All", user_id)) >= maxtask:
