...
from bot import aria2, app, LOGGER, DOWNLOAD_DIR, get_client, premium_session, config_dict, STORAGE_THRESHOLD, user_data, bot, main_loop
...
def exit_clean_up(signal, frame):
    try:
        ...
        main_loop.create_task(clean_all())
        sysexit(0)
...
async def split_file(path, size, file_, dirpath, split_size, listener, start_time=0, i=1, inLoop=False, noMap=False):
    ...
    leech_split_size = int((user_dict and user_dict.get(
    ...
    if get_media_streams(path)[0]:
        ...
        while i <= parts:
            ...
            if not noMap:
                listener.suproc = runShell(
                    f'ffmpeg -hide_banner -loglevel error -ss {str(start_time)} -i {path} -fs {str(split_size)} -map 0 -map_chapters -1 -c copy "{out_path}"')
            else:
                listener.suproc = runShell(
                    f'ffmpeg -hide_banner -loglevel error -ss {str(start_time)} -i "{path}" -fs {str(split_size)} -map_chapters -1 -c copy "{out_path}"')
        ...
        listener.suproc = await runShell(f'split --numeric-suffixes=1 --suffix-length=3 --bytes={split_size} "{path}" "{out_path}"')
        if listener.suproc.returncode == -9:
            return False
    return True
...
def getMediaInfo(path):
    process = srun(f'mediainfo "{path}"', capture_output=True, shell=True)
    reply = f"<h4>{ospath.basename(path)}</h4><br><br>"
    stderr = process.stderr.decode()
    stdout = process.stdout.decode()
    if len(stdout) != 0:
        reply += f"<br><br><pre>{stdout}</pre><br>"
    if len(stderr) != 0:
        reply += f"<br><br><pre>{stderr}</pre><br>"
    help = telegraph.create_page(title="MediaInfo", content=reply)["path"]
    return f"https://te.legra.ph/{help}"
