...
async def format_filename(file_, user_id, dirpath=None, isMirror=False):
    ...
    remname = config_dict[f'{ctag}_FILENAME_PREFIX'] if (val:=user_dict.get(f'{ftag}remname', '')) == '' else val
    suffix = config_dict[f'{ctag}_FILENAME_PREFIX'] if (val:=user_dict.get(f'{ftag}suffix', '')) == '' else val
