...
from asyncio import sleep as asleep, get_running_loop

from bot.helper.ext_utils.db_handler import DbManger
from bot.helper.themes import BotTheme
from bot.helper.telegram_helper.bot_commands import BotCommands
from bot.helper.telegram_helper.button_build import ButtonMaker
from bot import LOGGER, CATEGORY_IDS, CATEGORY_INDEX, CATEGORY_NAMES, DATABASE_URL, bot, download_dict, \
    ...
    async with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [MirrorStatus.STATUS_SEEDING, MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_CONVERTING, MirrorStatus.STATUS_QUEUEDL, MirrorStatus.STATUS_QUEUEUP]:
                msg += BotTheme(download.message.from_user.id).STATUS_MSG1.format(sm1=get_progress_bar_string(download), sm2=download.progress(), sm3=get_readable_file_size(download.processed_bytes()), \
                       sm4=download.size(), sm5=download.speed(), sm6=download.eta(), sm7=get_readable_time(time() - download.message.date.timestamp()), sm8=download.eng())
                if hasattr(download, 'seeders_num'):
                    try:
                        msg += BotTheme(download.message.from_user.id).STATUS_MSG2.format(sm9=download.seeders_num(), sm10=download.leechers_num(), sm11=BotCommands.BtSelectCommand[0], sm12=download.gid())
                    except:
                        pass
                if download.message.chat.type != ChatType.PRIVATE:
                    try:
                        chatid = str(download.message.chat.id)[4:]
                        msg += BotTheme(download.message.from_user.id).STATUS_MSG3.format(sm13='https://t.me/c/{chatid}/{download.message.id}', sm14=download.message.from_user.first_name, sm15=download.message.from_user.id, \
                               sm16=BotCommands.CancelMirror[0], sm17=download.gid())                
                    except:
                        pass
                else:
                    msg += BotTheme(download.message.from_user.id).STATUS_MSG4.format(sm18=download.message.from_user.first_name, sm19=download.message.from_user.id, sm20=BotCommands.CancelMirror[0], sm21=download.gid())
            elif download.status() == MirrorStatus.STATUS_SEEDING:
                msg += BotTheme(download.message.from_user.id).STATUS_MSG5.format(sm22=download.size(), sm23=download.upload_speed(), sm24=download.uploaded_bytes(), sm25=download.ratio(), sm26=download.seeding_time(), \
                       sm27=get_readable_time(time() - download.message.date.timestamp()), sm28=BotCommands.CancelMirror[0], sm29=download.gid())
            else:
                msg += BotTheme(download.message.from_user.id).STATUS_MSG6.format(sm30=download.eng(), sm31=download.size())
            msg += BotTheme(download.message.from_user.id).STATUS_MSG7
...
@bot.on_callback_query(filters.regex(pattern="^" + str(THREE) + "$"))
async def pop_up_stats(c: Client, query: CallbackQuery):
    stats = bot_sys_stats()
