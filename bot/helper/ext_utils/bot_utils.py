                ...
                if download.message.chat.type != 'private':
                    ...
                    if config_dict['EMOJI_THEME']:
                       # msg += f'\n<b>├👤 User:</b> ️<code>{download.message.from_user.first_name}</code> | <b>Id:</b> <code>{download.message.from_user.id}</code>'
                        msg += f"\n<b>╰❌ </b><code>/{BotCommands.CancelMirror_download.gid()}</code>"
                    else:
                        msg += f'\n<b>├ User:</b> ️<code>{download.message.from_user.first_name}</code> | <b>Id:</b> <code>{download.message.from_user.id}</code>'
                        msg += f"\n<b>╰ Cancel: </b><code>/{BotCommands.CancelMirror} {download.gid()}</code>"
    ...
    if user_id in user_data and user_data[user_id].get('is_usertd') and u_index is not None:
        ...
        if len(GDNames) != 0:
            for j, _gname in enumerate(GDNames):
                buttons.sbutton(f'{_gname} {"✅️" if u_index is not None and _gname == GDNames[u_index] else ""}', f"{query_data} ucat {msg_id} {j}")
    else:
        text += f"\n<b>Upload</b>: To Drive in {CATEGORY_NAMES[c_index]} folder"
        for i, _name in enumerate(CATEGORY_NAMES):
            buttons.sbutton(f'{_name} {"✅️" if u_index is None and _name == CATEGORY_NAMES[c_index] else ""}', f"{query_data} scat {msg_id} {i}")
